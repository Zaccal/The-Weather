    ...
    return (
        <Container className="py-5 flex items-center h-[70vh] justify-center">
            <div className="max-w-lg w-full">
                ...
                {searchValueError && <Label className='text-red-400' htmlFor='searchInput'>Search input must contain the value</Label>}
                <Command className="mt-2 rounded-lg border shadow-lg">
                    <CommandInput id='searchInput' value={searchValue} onValueChange={searchHandler} placeholder="Enter your location..."/>
                    <CommandList>
                        {
                                   ...
                                   return (
                                        <CommandItem
                                            ...
                                            onSelect={currentValue => {
                                                setIsOpen(false)                                                
                                                setSearchValue(currentValue.split(' ').map(word => word.charAt(0).toLocaleUpperCase() + word.slice(1)).join(' '))
